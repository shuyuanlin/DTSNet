...
def get_args():
    ...
    parser.add_argument('--checkpoint_folder', default = './experiments/MVTec-AD/N+A--PR+2T1S', type=str)
    ...
    with torch.no_grad():
        for (img, gt, label, _, _) in test_dataloader:
            if count == 79:
                ...
                outputs = APR_module(outputs, inputs)
                ...
                plt.imsave(_class_ + '_anomaly_map.png', ano_map)
                plt.imsave(_class_ + '_gt.png', gt, cmap='gray')
                plt.imsave(_class_ + '_img.png', img)
...
if __name__ == '__main__':
    pars = get_args()
    # 104, 5, 92, 3, 81, 1
    item_list = ['leather']    # 'tile', 'cable', 'hazelnut', 'pill', 'transistor', 'zipper'
    setup_seed(111)
    for c in item_list:
        vis(c, pars)
