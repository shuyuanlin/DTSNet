...
from dataset.dataset import VisADataset_train, VisADataset_test
...
def get_args():
    ...
    parser.add_argument('--classes', nargs="+", default=['chewinggum'])
    parser.add_argument('--save_folder', default='./experiments/VisA', type=str)
    parser.add_argument('--dataset_path', default='/home/sylin/LTX/datasets/VisA', type=str)
...
def train(_class_, pars):
    ...
    test_data = VisADataset_test(dataset_path=test_path, resize=(pars.image_size, pars.image_size))
    ...
    optimizer_rec = torch.optim.Adam(list(APR_module.parameters()), lr=pars.rec_lr, betas=(0.5, 0.999))
    ...
    epoch_dict = {
        'chewinggum': 10,
        'capsules': 200, 'macaroni1': 200, 'macaroni2': 200, 'pcb3': 200, 'pcb4': 200, 'pipe_fryum': 200,
        'candle': 250, 'cashew': 250, 'fryum': 250, 'pcb1': 250,
        'pcb2': 300
    }
...
if __name__ == '__main__':
    setup_seed(111)
    pars = get_args()
    visa_classes = ['chewinggum', 'candle', 'capsules', 'cashew', 'fryum', 'macaroni1', 'macaroni2', 'pcb1', 'pcb2', 'pcb3', 'pcb4', 'pipe_fryum']

    metrics = {'class': [], 'AUROC_sample': [], 'AUROC_pixel': [], 'AUPRO_pixel': []}

    print('Training with classes: ', visa_classes)
